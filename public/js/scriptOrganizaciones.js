...
function Carga(){
	...
	$.get(route, function(res){
		$(res).each(function(key, value){
			tablaDatos.append("<tr><td>"+value.nombre+"</td><td>"+value.tipo_organizacion+"</td><td>"+value.telefono+"</td><td>"+value.sitio_web+"</td><td>"+value.twitter+"</td><td><button value="+value.id+" OnClick='Mostrar(this);' class='btn btn-primary' data-toggle='modal' data-target='#myModal'>Editar</button></td><td><a href='/app/crear_reporte_organizacion/1/"+value.id+"' target='bland_' value="+value.id+" OnClick='MostrarIdReporte(this);' class='btn btn-success'>Generar Reporte</a></td></tr>");
